...
- June 28, 2025. Fixed admin navigation logic: gender-specific admins (AdminB and AdminG) now bypass the admin selection page and go directly to their respective admin dashboards, while super admin (Admin) still gets the selection page to choose between complexes. Removed duplicate navigation buttons and fixed green color artifacts in login form by updating CSS variables to use slate/gray color scheme consistently.